document.addEventListener("DOMContentLoaded", function () {
  ...
  const startSlider = () => {
    ...
    sliderContainer.style.transform = `translateX(-${(currentIndex + 1) * (100 / slidesToShow)}%)`; // Adjust based on screen size
  };
  ...
  const updateSliderPosition = () => {
    ...
    sliderContainer.style.transform = `translateX(-${(currentIndex + 1) * (100 / slidesToShow)}%)`;
  };
  ...
  const resetSliderPosition = () => {
    ...
    // When moving backward, check if we've reached the first clone and reset to the last real slide
      ...
      sliderContainer.style.transform = `translateX(-${(sliderCount - slidesToShow + 1) * (100 / slidesToShow)}%)`;
    }
